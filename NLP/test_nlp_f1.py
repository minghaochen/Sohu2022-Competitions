...
from CirBERTa.DeBERTa.apps.models.masked_language_model import MaskedLanguageModel
accelerator = Accelerator()

import argparse
parser = argparse.ArgumentParser()
parser.add_argument("--local_rank", default=-1)
# parser.add_argument("--model_name", default='./paperlog/CirBERTa/4_0.9916390370477152_modelpytorch_model.bin')
parser.add_argument("--model_name", default='./paperlog/CirBERTa/3_0.9858493920216169_modelpytorch_model.bin')
...
with open(r'Sohu2022_data/nlp_data/nlp-test-dataset.txt',encoding='utf-8') as f:
...
np.save('pred_test_prob_1', pred.numpy())
...
for i in range(len(valid)):
    ...
    for entity in valid[i]['entity']:
        # es[entity] = torch.tensor([1.00995423,1.05855194,1.00611749,0.9031945,1.05422266]*pred[n].numpy()).argmax().item()-2 #pred[n].argmax().item()-2
        # es[entity] = torch.tensor([1.1,1.0,1.0,1.0,1.15]*pred[n].numpy()).argmax().item()-2 #pred[n].argmax().item()-2
        # es[entity] = torch.tensor([1.03921836, 0.99067226, 0.98160849,0.97160117, 1.03890902]*pred[n].numpy()).argmax().item()-2 
        # best
        # es[entity] = torch.tensor([1.05931229, 0.9827048,  0.92778563, 0.97837141, 1.05778381]*pred[n].numpy()).argmax().item()-2
        # final1
        # es[entity] = torch.tensor([1.05716332, 0.98645722, 0.93372656, 0.97632701, 1.05683334]*pred[n].numpy()).argmax().item()-2
        # final 2
        es[entity] = torch.tensor([1.07149659, 0.98724847, 0.95074389, 0.96523225, 1.0605685 ]*pred[n].numpy()).argmax().item()-2
        
        

        
        #pred[n].argmax().item()-2     
        n+=1
