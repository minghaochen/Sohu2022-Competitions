...
parser.add_argument("--model_name", default='./paperlog/CirBERTa_final_pseudo_2/4_0.9871457993040267_modelpytorch_model.bin')
...
# with open(r'Sohu2022_data/nlp_data/test.txt',encoding='utf-8') as f:
...
for i in range(len(valid)):
    ...
    for entity in valid[i]['entity']:
        temp = torch.nn.Softmax(dim=0)(torch.tensor([1.03921836, 0.99067226, 0.98160849,0.97160117, 1.03890902]*pred[n].numpy())).numpy()
        #(pred[n]).numpy()
        # if (np.max(temp) > 0.9) and (pred[n].argmax().item()-2 != 0):
            # es[entity] = pred[n].argmax().item()-2
        if np.max(temp) > 0.85:
            es[entity] = pred[n].argmax().item()-2
        n+=1
    if len(list(set([-2,-1,1,2]).intersection(set(es.values())))) > 0:
    # if len(es) > 0:
        result.append({'id':valid[i]['id'],'content': valid[i]['content'],'entity':es})

pd.to_pickle(result,'pseudo_nlp_2.pk')
