...
pd.to_pickle(datas,'train_final_f1_opt.pk')
    
valid = pd.read_pickle('train_final_f1_opt.pk')


label_name = {}
num = 0
tokenizer = DebertaV2Tokenizer.from_pretrained(CFG['model'])
turn = {-2:tokenizer('坏').input_ids[2:-1],
        -1:tokenizer('差').input_ids[2:-1],
        0:tokenizer('平').input_ids[2:-1],
        1:tokenizer('行').input_ids[1:-1],
        2:tokenizer('好').input_ids[2:-1]}
print(turn)
...
criterion = LabelSmoothingCrossEntropy(reduction='sum') #nn.CrossEntropyLoss()
...
print(np.array(y_truth).shape)


from sklearn.metrics import f1_score
from functools import partial
import scipy as sp
print('before f1:', f1_score(y_truth, np.argmax(pred,axis=1), average='macro'))
def f1_opt(weight, y_hat, y):
    y_hat = weight*y_hat
    loss = f1_score(y, np.argmax(y_hat,axis=1), average='macro')
    return -loss
def get_weight(y_hat, y):
    loss_partial = partial(f1_opt, y_hat=y_hat, y=y)
    initial_weights = [1. for _ in range(5)]
    weights_ = sp.optimize.minimize(loss_partial, initial_weights, method='Nelder-Mead')
    return weights_['x']
weights = get_weight(pred, y_truth)
print('weight:', weights)
print('after f1:', f1_score(y_truth, np.argmax(weights*pred,axis=1), average='macro'))