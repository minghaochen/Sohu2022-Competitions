...
train = pd.read_pickle('train_nlp_1.pk')
print('len of train', len(train))
valid = pd.read_pickle('valid_nlp.pk')

# 伪标签
pseudo1 = pd.read_pickle('pseudo_nlp_1.pk')
print('len of pseudo1', len(pseudo1))
train = train + pseudo1
print('len of train', len(train))
pseudo2 = pd.read_pickle('pseudo_nlp_2.pk')
print('len of pseudo2', len(pseudo2))
train = train + pseudo2
...
def train_model(model, fgm,pgd,train_loader,epoch):
    ...
    for step, (input_ids, attention_mask,y) in enumerate(tk):
        with autocast():
            ...
            logits2[:,:64243]=0
            logits2[:,64244:68158]=0
            logits2[:,68159:110093]=0
            logits2[:,110094:115270]=0
            logits2[:,115271:125883]=0
            logits2[:,125884:]=0
            
            ce_loss = 0.5 * (criterion(logits, output['labels']) + criterion(logits2, output2['labels']))
